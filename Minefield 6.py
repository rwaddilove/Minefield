...
def show_dug():
    ...
    print("   ", end="")
    for i in range(size):
        print(f"{i:>2} ", end="")         # modify to format within space to allow double digits
    print("\n  ", "---" * size)
...
def get_move(turn) -> str:
    ...
    if turn == 0 and board[row][col] == 9:
        board[row][col] = 0
        board[row][col] = nearby_bombs(row,col)   # don't die on first turn!
...
def show_clear_cells(row,col) -> None:
...
def check_minefield():
    ...
    print(f"\nThere are {bombs} bombs. You flagged {correct} correctly.")
    if correct == bombs:
        print("Congratulations! You have done it!")
        return 'Done!'
    if incorrect > 0:
        print(f"There are {incorrect} incorrect flags.")
    return 'OK'
...
while play == 'OK':
    ...
    play = get_move(turns)   # returns 'OK' or 'Bomb!'
    turns += 1
    if play == 'Bomb!':
